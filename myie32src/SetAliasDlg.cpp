...
BOOL CSetAliasDlg::OnInitDialog() 
{
	...
	POSITION pos = ((CMainFrame*)pMainFrame)->m_mapAlias.GetStartPosition();
	CString alias, url;
	int i=0;
	while(pos!=NULL)
	{
...
void CSetAliasDlg::OnOK() 
{
	...
	((CMainFrame*)pMainFrame)->m_mapAlias.RemoveAll();
	...
	for( int i=0; i<n; i++)
	{
		...
		((CMainFrame*)pMainFrame)->m_mapAlias.SetAt(alias,m_list.GetItemText(i,1));
	}
